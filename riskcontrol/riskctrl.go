...
func (this *RiskCtrl) baseCtrl(exname string) {
    ...
    for {
        ...
        if long.Amount > 0 {
            ...
            rows = append(rows, fmt.Sprintf("L[%.0f %.4f %.1f%%]", long.Amount, lprofit, lrop * 100))
            hasPosition = true
        } else {
            lMaxRop = 0
        }
        if short.Amount > 0 {
            ...
            rows = append(rows, fmt.Sprintf("S[%.0f %.4f %.1f%%]", short.Amount, sprofit, srop * 100))
            hasPosition = true
        } else {
            sMaxRop = 0
        }
        ...
        if long.AvailableAmount > 0 && lrop < -0.10 {
            ok.Trade(exsync.TradeAction_CloseLong, long.AvailableAmount, 0)
        ...
        if short.AvailableAmount > 0 && srop < -0.10 {
            ok.Trade(exsync.TradeAction_CloseShort, short.AvailableAmount, 0)
